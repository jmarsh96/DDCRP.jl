...
@testset "Core Utilities" begin
    ...
    @testset "MCMCOptions" begin
        ...
        @test MixedFixedDim(λ = WeightedMean(), α = NoUpdate()) isa FixedDimensionProposal

        # Test infer_params dictionary
        opts_custom = MCMCOptions(infer_params=Dict(:α_ddcrp => false, :s_ddcrp => false))
        @test should_infer(opts_custom, :λ) == true
        @test should_infer(opts_custom, :α_ddcrp) == false
        @test should_infer(opts_custom, :s_ddcrp) == false
    end
    ...
    @testset "MCMCOptions - Comprehensive" begin
        ...
        @test should_infer(opts, :α_ddcrp) == false
        @test should_infer(opts, :s_ddcrp) == false

        # Test get_prop_sd
        @test get_prop_sd(opts, :λ) == 0.1
        @test get_prop_sd(opts, :α) == 0.3

        # Default prop_sd when not specified
        opts_default = MCMCOptions()
        @test get_prop_sd(opts_default, :λ) > 0
        ...
        fdim = MixedFixedDim(λ = WeightedMean(), α = NoUpdate())
        @test fdim.proposals.λ isa WeightedMean
        @test fdim.proposals.α isa NoUpdate
    end
